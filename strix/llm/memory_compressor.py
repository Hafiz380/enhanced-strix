...
SUMMARY_PROMPT_TEMPLATE = """You are an agent performing ECC-style (Everything Claude Code) context
...
ECC STRATEGIC COMPACT PRINCIPLES:
1.  **Deduplication**: Collapse repetitive findings into a single canonical entry with a count.
2.  **Hypothesis Preservation**: Keep all currently active security hypotheses and their validation status.
3.  **Path Traversal State**: Maintain the current position in the target application's route/file tree.
4.  **Tool State**: Store relevant state from proxy, browser, and terminal sessions that sub-agents need.
5.  **Chain of Thought**: Preserve the core reasoning that led to current testing paths.
